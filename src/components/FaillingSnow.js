function setup(){
  ...
  function drawSnowball(canvasContext,snowBall){
    ...
    canvasContext.fillStyle = `rgba(147,112,219,${snowBall.opacity})`
    canvasContext.fill();
  }
  
  function run(){
    const {canvas,canvasContext,numberOfSnowBalls} = setup();
    const snowBall = createSnowBalls(canvas,numberOfSnowBalls)
    
    setInterval(()=>{
      canvasContext.clearRect(0,0,canvas.width,canvas.height)
      snowBall.forEach((snowBall)=>drawSnowball(canvasContext,snowBall))
